...
static void erase_flash_chip_sector(uint8_t * page_address,
		uint8_t size_of_page_adress);
...
void packet_store_write_buffer_to_page(uint8_t * buffer, uint16_t length) {
	...
	//send the command
	HAL_SPI_Transmit(&SpiHandle, &page_program_cmd, sizeof(page_program_cmd),
			SPI_DEFAULT_TIMEOUT); //send command
	//send addresses
	HAL_SPI_Transmit(&SpiHandle, page_address, sizeof(page_address),
			SPI_DEFAULT_TIMEOUT);	//send command
	//send data
...
uint8_t packet_store_read_data_to_buffer(uint8_t * read_buffer,
	...
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
	//send the command
	HAL_SPI_Transmit(&SpiHandle, &read_data_cmd, sizeof(read_data_cmd),
			SPI_DEFAULT_TIMEOUT);	//send command
	//send addresses
	HAL_SPI_Transmit(&SpiHandle, page_address, sizeof(page_address),
			SPI_DEFAULT_TIMEOUT);	//send command
...
static void get_flash_chip_id(void) {
	...
	READ_UNIQUE_ID, 0x00, 0x00, 0x00, 0x00	//DUMMY BYTES
			};

	//TODO write/find generic function that takes the input and output buffer to summarize the next 4 lines
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);

	HAL_SPI_Transmit(&SpiHandle, device_id_cmd, sizeof(device_id_cmd),
			SPI_DEFAULT_TIMEOUT);	//send command
	HAL_SPI_Receive(&SpiHandle, device_id, sizeof(device_id),
			SPI_DEFAULT_TIMEOUT);	//read back response
...
static void get_flash_chip_manufacturer_id(void) {
	uint8_t device_id_cmd[] = {
	READ_MANUFACTURER_ID, 0x00, 0x00, 0x00	//address zero
			};
	...
	HAL_SPI_Transmit(&SpiHandle, device_id_cmd, sizeof(device_id_cmd),
			SPI_DEFAULT_TIMEOUT);	//send command
	HAL_SPI_Receive(&SpiHandle, manufacturer_id, sizeof(manufacturer_id),
			SPI_DEFAULT_TIMEOUT);	//read back response
...
static void flash_chip_write_enable(void) {
	...
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&SpiHandle, &write_enable_command,
			sizeof(write_enable_command), SPI_DEFAULT_TIMEOUT);	//send command
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
...
static void wait_for_flash_to_be_ready(void) {
	...
	do {
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
		//send the command
		HAL_SPI_Transmit(&SpiHandle, &read_status_cmd, sizeof(read_status_cmd),
				SPI_DEFAULT_TIMEOUT);	//send command
		HAL_SPI_Receive(&SpiHandle, response, sizeof(response),
				SPI_DEFAULT_TIMEOUT);	//read back response
...
static void erase_flash_chip_sector(uint8_t * page_address,
	...
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
	//send the command
	HAL_SPI_Transmit(&SpiHandle, &sector_erase_cmd, sizeof(sector_erase_cmd),
			SPI_DEFAULT_TIMEOUT);	//send command
	//send addresses
	HAL_SPI_Transmit(&SpiHandle, page_address, size_of_page_adress,
			SPI_DEFAULT_TIMEOUT);	//send command
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
