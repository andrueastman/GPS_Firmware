...
void parse_gps_info();
void build_http_message(char * message, uint16_t data_length,
		uint8_t * filedata);
...
void gsm_tick() {
	...
	switch (gsm_state) {
	case STATE_RESET:
		uart_gsm_send(factory_reset, sizeof(factory_reset) - 1,
				sizeof(factory_reset) + 5); //expect command back plus OK
		if (0
				== memcmp(aRxBuffer + sizeof(factory_reset) - 1, ok_response,
						sizeof(ok_response) - 1)) {
			gsm_state++;
		}
		break;
	case STATE_DISABLE_ECHO:
		uart_gsm_send(disable_echo, sizeof(disable_echo) - 1,
				sizeof(disable_echo) + 5); //expect commandd back plus OK
		if (0
				== memcmp(aRxBuffer + sizeof(disable_echo) - 1, ok_response,
						sizeof(ok_response) - 1)) {
			gsm_state++;
		}
		break;
	...
	case STATE_SET_AGPS_URL:
		uart_gsm_send(set_AGPS_URL, sizeof(set_AGPS_URL) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			gsm_state++;
		}
	...
	case STATE_SET_AGPS_SSL:
		uart_gsm_send(set_AGPS_SSL, sizeof(set_AGPS_SSL) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			gsm_state++;
		}
		break;
	case STATE_GPS_STOP:
		uart_gsm_send(turn_off_AGPS, sizeof(turn_off_AGPS) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			gsm_state++;
		}
	...
	case STATE_GPS_START:
		uart_gsm_send(turn_on_AGPS, sizeof(turn_on_AGPS) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
	...
	case STATE_GET_GPS:
		if (gps_reads < 50) {
			if (gps_read_period % 5 == 0) {
				...
				if (0
						== memcmp(aRxBuffer, gps_info_resp,
								sizeof(gps_info_resp) - 1)) {
					parse_gps_info();
					gps_reads++;
				}
			}
	...
	case STATE_ATTACH_GPRS:
		//try to detech first
		uart_gsm_send(gprs_dettach, sizeof(gprs_dettach) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			uart_gsm_send(gprs_attach, sizeof(gprs_attach) - 1,
					sizeof(ok_response) - 1);
			if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
				gsm_state++;
			}
		...
		break;
	case STATE_CGDCONT:
		uart_gsm_send(set_CGDCONT, sizeof(set_CGDCONT) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			gsm_state++;
		}
	...
	case STATE_START_HTTPS_SESSION:
		uart_gsm_send(start_https, sizeof(start_https) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			gsm_state++;
		} else {
			uart_gsm_send(stop_https, sizeof(stop_https) - 1,
					sizeof(ok_response) - 1);
			gsm_state = STATE_ATTACH_GPRS;
		}
		break;
	case STATE_OPEN_HTTPS_SESSION:
		uart_gsm_send(open_https, sizeof(open_https) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
			gsm_state++;
		} else {
	...
	case STATE_SEND_HTTPS_DATA:
		send_flag = packet_store_read_data_to_buffer(temp_buffer,
		...
		if (1 == send_flag) {
			...
			uart_gsm_send((uint8_t *) command, strlen(command),
					sizeof(ready_to_send) - 1);

			if (0
					== memcmp(aRxBuffer, ready_to_send,
							sizeof(ready_to_send) - 1)) {
				//memcpy(&message[strlen(message)],temp_buffer,sizeof(temp_buffer));
				uart_gsm_send((uint8_t *) message, strlen(message),
						sizeof(connect_ok) - 1);
				gsm_state++;
			}
		} else {
	...
	case STATE_RECEIVE_HTTPS_RESPONSE:
		uart_gsm_send(receive_http_response, sizeof(receive_http_response) - 1,
				sizeof(ok_response) - 1);
		if (0 == memcmp(aRxBuffer, ok_response, sizeof(ok_response) - 1)) {
