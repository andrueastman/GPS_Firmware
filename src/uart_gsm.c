...
void uart_gsm_send(uint8_t *  tx_buffer, uint16_t send_length, uint16_t receive_length)
{
	...
	if(receive_length != 0)
	{
		if(HAL_UART_Receive_IT(&UartHandle, (uint8_t *)aRxBuffer, receive_length) != HAL_OK)
		{
			Error_Handler();
		}
